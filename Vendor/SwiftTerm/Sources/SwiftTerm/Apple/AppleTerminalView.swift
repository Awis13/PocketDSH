...
extension TerminalView {
    ...
    func getAttributes (_ attribute: Attribute, withUrl: Bool) -> [NSAttributedString.Key:Any]?
    {
        ...
        var nsattr: [NSAttributedString.Key:Any] = [
            .font: tf,
            .foregroundColor: flags.contains(.dim) ? fgColor.withAlphaComponent(0.6) : fgColor,
    ...
    func buildAttributedString (row: Int, line: BufferLine, cols: Int, prefix: String = "") -> ViewLineInfo
    {
        ...
        res.append (NSAttributedString(string: str, attributes: getAttributes(attr, withUrl: hasUrl)))
        #if !os(macOS)
        if let characterFont {
            var offset = 0
            for character in res.string {
                let length = String(character).utf16.count
                if let font = characterFont(character) {
                    res.addAttribute(.font, value: font, range: NSRange(location: offset, length: length))
                }
                offset += length
            }
        }
