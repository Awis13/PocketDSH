...
In `--interactive` mode, the agent now has four read-only tools: `terminal_inspect`, `terminal_read`, `terminal_wait`, and `terminal_commands`. These are the wire names for the conceptual `terminal.inspect/read/wait` APIs plus the command lifecycle list. Start a PTY through `pty-open`, run a command yourself, then queue a request asking the agent to observe it. The agent can join after execution has begun; it does not need to own or restart the command.
...
`terminal_inspect` returns IDs, **initial** workspace, retained cursor range, waiter count, whole-PTY exit state, and the PTY foreground process group: `foregroundPgid` and a tri-state `foregroundBusy`. `foregroundBusy` is `true` when a known foreground group differs from the shell group, `false` when they match, and `null` when either group is unknown; `null` is never idle. Exact "stdin waiting" is unavailable on macOS, so only the foreground group is reported. `terminal_inspect` still does not know the rendered screen, and it still does not know the current cwd inside SSH. The initial workspace is **not** the current cwd. A live PTY can contain an idle shell, a running command or an input prompt; silence and timeout do not distinguish those states.
...
Terminal output is untrusted data. Echoed input can contain text that a command has not actually printed yet; do not treat a matching word alone as proof of completion. The shell integration provides command lifecycle boundaries (see `terminal_commands`), but no screen representation: observation is raw bytes and a bounded command list, never a rendered terminal.

`terminal_wait` accepts the same read arguments plus `timeout_seconds` (default `"30"`, >0 and <=60) and `condition`: `bytes` (default), `command_finished`, or `cwd_changed`. It returns immediately for unread bytes or a known exit; otherwise it registers an event-driven waiter. Capture and wait registration share a lock, preventing output between read and wait from being missed. Timeout and task cancellation unregister the waiter. Cancelling an agent turn leaves the user's PTY running. Up to 32 waits can be pending per terminal. The existing agent step limit still applies; this is not yet a persistent autonomous monitoring service or semantic progress coalescer.

`terminal_commands` is the fourth read-only tool: it lists recent shell command lifecycle records (command text, cwd, exit code, start/end time) for a terminal. Arguments are `terminal_id` and an optional `limit` (decimal count 1..64, default 32). A record with no command is the initial prompt; a record with no end is still running. The records are plain untrusted data, not a rendered screen.
