...
Subsequent increment, 2026-09-08: **NH-CONTEXT C1–C2 are implemented** — frozen provider requests, usage normalization, optional llama.cpp count/props, configured limits, estimated fallback, a shared Shell/Chat context indicator and retained request diagnostics. C1 was pushed to `codex/native-context-compaction`; C2 is a local increment. [Request accounting and diagnostics](NATIVE-CONTEXT.md) records configuration and verification. C3–C5 context projection/compaction are still pending.
